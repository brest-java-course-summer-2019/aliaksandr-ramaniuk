...
//    private static final String EMPLOYEE_ID = "employeeId";
    ...
    public void filterEmployee() throws Exception {
        ...
        Mockito.when(employeeService.filterEmployee("L")).thenReturn(Arrays.asList(createEmployeeForTest(EMPLOYEE_ID_1), createEmployeeForTest(EMPLOYEE_ID_2)));

        mockMvc.perform(MockMvcRequestBuilders
                .get("/employees2/{lastName}", "L")
                .accept(MediaType.APPLICATION_JSON_UTF8))
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(jsonPath("$", hasSize(2)))
        ...
        Mockito.verify(employeeService).filterEmployee("L");
        Mockito.verifyNoMoreInteractions(employeeService);
    ...
    public void filterEmployeeByDate() throws Exception {
        ...
        mockMvc.perform(MockMvcRequestBuilders
                .get("/employees/{localDateStart}/{localDateEnd}", localDateStart, localDateEnd)
                .accept(MediaType.APPLICATION_JSON_UTF8))
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(jsonPath("$", hasSize(2)))
        ...
        Mockito.verify(employeeService).filterEmployeeByDate(localDateStart, localDateEnd);
        Mockito.verifyNoMoreInteractions(employeeService);
